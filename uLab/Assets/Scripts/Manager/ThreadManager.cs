	...
	public class ThreadManager : Manager
	{
		...
		private void ProgressChanged(object sender, DownloadProgressChangedEventArgs e)
		{
			...
			*/
			...
			NotiData data = new NotiData(MessageDefine.UPDATE_PROGRESS, value);
			if (m_SyncEvent != null)
				m_SyncEvent(data);
			...
			if (e.ProgressPercentage == 100 && e.BytesReceived == e.TotalBytesToReceive)
			{
				...
				data = new NotiData(MessageDefine.UPDATE_DOWNLOAD, currDownFile);
				if (m_SyncEvent != null)
					m_SyncEvent(data);
			}
		}
		...
		void OnExtractFile(List<object> evParams)
		{
			...
			///------------------通知更新面板解压完成--------------------
			NotiData data = new NotiData(MessageDefine.UPDATE_DOWNLOAD, null);
			if (m_SyncEvent != null)
				m_SyncEvent(data);
		}
